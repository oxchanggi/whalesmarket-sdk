...
import { TIP_ACCOUNTS } from "./constants";
...
export class JitoJsonRpcClient {
  ...
  constructor(baseUrl: string, uuid: number | null = null) {
  ...
  async sendRequest(endpoint: string, method: string, params: any[] = []) {
  ...
  async sendBundle(params: any[]) {
    const endpoint = this.uuid ? `/bundles?uuid=${this.uuid}` : "/bundles";
    return this.sendRequest(endpoint, "sendBundle", params);
  }

  async sendTxn(params: any[], bundleOnly = false) {
  ...
  async getInFlightBundleStatuses(params: any[]) {
    const endpoint = this.uuid ? `/bundles?uuid=${this.uuid}` : "/bundles";
    return this.sendRequest(endpoint, "getInflightBundleStatuses", params);
  }

  async getBundleStatuses(params: any[]) {
    const endpoint = this.uuid ? `/bundles?uuid=${this.uuid}` : "/bundles";
